...
function CMDsGpu:initialize(_gpu)
  ...
  if totalMemory > Memory_1T * 5 then
    MAX_CMDs_COUNT = 1000
  end
  if totalMemory > Memory_1T * 6 then
    MAX_CMDs_COUNT = 1500
  end
  if totalMemory > Memory_1T * 8 then
    MAX_CMDs_COUNT = 2000
  end
...
function CMDsGpu:setCmdType(cmd_type)  
  if cmd_type_id[cmd_type] ~= nil then
	...
	self:writeCMDsCache("info", "["..os.date("%X").."]INFO:Change CMD Type to "..cmd_type)
    self:printCMDsCache()   
